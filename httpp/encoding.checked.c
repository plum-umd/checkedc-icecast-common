...
static void __flush_output(_Ptr<httpp_encoding_t> self, _Ptr<ssize_t (void* , const void* , size_t )> cb, void* userdata)
{
...
httpp_meta_t * httpp_encoding_meta_new(_Nt_array_ptr<const char> key, _Nt_array_ptr<const char> value)
{
...
int httpp_encoding_release(httpp_encoding_t *self : itype(_Ptr<httpp_encoding_t> ) )
{
    if (!self)
        return -1;
 ...
 */
ssize_t httpp_encoding_read(_Ptr<httpp_encoding_t> self, void *buf, size_t len, _Ptr<ssize_t (void* , void* , size_t )> cb, void* userdata)
{
    ssize_t done = 0;
    ssize_t ret;
...
int httpp_encoding_eof(_Ptr<httpp_encoding_t> self, _Ptr<int (void* )> cb, void* userdata)
{
    if (!self)
        return -1;
 ...
 */
httpp_meta_t * httpp_encoding_get_meta(_Ptr<httpp_encoding_t> self)
{
    httpp_meta_t *ret;
...
static void __enc_chunked_read_extentions(_Ptr<httpp_encoding_t> self, char *p, size_t len)
{
...
static size_t __enc_chunked_write_extensions_valuelen(httpp_meta_t *cur : itype(_Ptr<httpp_meta_t> ) )
{
...
static char * __enc_chunked_write_extensions(_Ptr<httpp_encoding_t> self)
{
    size_t buflen;
    void *buf;
    char *p;
